...
export class UserRepository implements IUserRepository {
	...
	getById(id: number): Promise<UserModel> {
		return this.db.select("*").from(tableName.USERS).where({ id }).then((d) => d[0])
	}
	create(data: UserModel): Promise<UserModel> {
		return this.db.insert(data, "*").into(tableName.USERS).then((d) => d[0])
	}
	async update(data: Partial<UserModel>): Promise<UserModel> {
		...
		return this.db.update(data, "*").where({ id: data.id }).into(tableName.USERS).then((d) => d[0])
	}
	async delete(id: number): Promise<boolean> {
		...
		return this.db.where(id).from(tableName.USERS).del()
	}
}
